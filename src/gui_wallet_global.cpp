...
void Globals::setCommandLine(bpo::options_description &app_options, bpo::options_description &cfg_options)
{
   ...
   cli.add_options()
      ...
      ("wallet-file,w", bpo::value<boost::filesystem::path>()->default_value(
          (graphene::utilities::decent_path_finder::instance().get_decent_home() / "wallet.json")), "Wallet to load.")
...
void Globals::startDaemons(BlockChainStartType type, const boost::filesystem::path &wallet_file, const graphene::wallet::server_data &ws)
{
...
void Globals::slot_connected(std::string const& str_error)
{
   ...
   if (str_error.empty()) {
      ...
      m_blockStart = std::chrono::system_clock::from_time_t(getWallet().exec(&graphene::wallet::wallet_api::head_block_time).wait().sec_since_epoch());
   }
   else
...
void Globals::slot_timer()
{
   ...
   {
      auto currBlockTime = std::chrono::system_clock::from_time_t(getWallet().exec(&graphene::wallet::wallet_api::head_block_time).wait().sec_since_epoch());
